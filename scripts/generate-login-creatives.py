...
def reach(draw, progress):
    draw.rectangle((0, 0, WIDTH, HEIGHT), fill="#4f46e5")
    ...
    draw.ellipse((cx - radius, cy - radius, cx + radius, cy + radius), fill="#c2e834")
    draw.ellipse((-150, 370, 430, 950), fill="#3329bd")
    draw.text((58, 50), "BLOOM / CTV", font=font(25, True), fill="#dfe2ff")
    draw.text((58, 185), "БРЕНД", font=font(89, True), fill="white")
    draw.text((58, 277), "В ЦЕНТРЕ", font=font(89, True), fill="white")
    draw.text((58, 369), "ВНИМАНИЯ", font=font(89, True), fill="#c2e834")


def channels(draw, progress):
    draw.rectangle((0, 0, WIDTH, HEIGHT), fill="#c2e834")
    offset = int(progress * 170)
    for index in range(-2, 9):
        x = index * 150 + offset
        draw.line((x, -40, x - 320, 580), fill="#b1d725", width=42)
    draw.text((58, 52), "ОДИН КАБИНЕТ", font=font(25, True), fill="#4338ca")
...
def results(draw, progress):
    ...
    for index in range(9):
        ...
        color = "#c2e834" if index > 5 else "#4f46e5"
        draw.rounded_rectangle((x, base - height, x + 64, base), radius=18, fill=color)
    draw.rectangle((0, 0, WIDTH, 305), fill="#17161c")
    draw.text((58, 50), "РЕЗУЛЬТАТЫ В РЕАЛЬНОМ ВРЕМЕНИ", font=font(24, True), fill="#a5abfb")
    draw.text((58, 128), "61 МЛН", font=font(112, True), fill="white")
    draw.text((60, 244), "ПОКАЗОВ ЗА МЕСЯЦ", font=font(31, True), fill="#c2e834")
...
def main():
    ...
    make_video("bloom-reach.mp4", reach)
    make_video("bloom-channels.mp4", channels)
    make_video("bloom-results.mp4", results)
